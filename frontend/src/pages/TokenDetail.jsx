        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {token.creator_handle ? (
                ...
                <div className="text-[10px] text-[#52525B] italic mt-1">creator X handle unresolved</div>
              )}
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4 text-right">
            <div>
              ...
              <div className="text-[#00FF66] font-bold glow-green">{formatEth(token.lifetime_earned_weth || token.total_claimed_eth || 0)} Ξ</div>
              <div className="text-[10px] text-[#52525B]">{formatUsd((token.lifetime_earned_weth || token.total_claimed_eth || 0) * 2094)}</div>
            </div>
            <div>
              <div className="text-[10px] tracking-widest text-[#52525B] uppercase">Total Claims</div>
              <div className="text-[#FFE600] font-bold">{token.total_claim_count || 0}</div>
            </div>
            <div>
              <div className="text-[10px] tracking-widest text-[#52525B] uppercase">Share</div>
              <div className="text-white font-bold">{token.share || "57%"}</div>
            </div>
          </div>
        </div>
        ...
        <div className="panel corners">
          ...
          <div className="divide-y divide-[#00FF66]/10 max-h-[300px] overflow-auto">
            ...
            {recent_claims.map((c) => (
              <Link
                key={c.id}
                to={c.claimer_handle ? `/handle/${c.claimer_handle}` : `/wallet/${c.claimer_wallet}`}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-white truncate">
                    {c.claimer_handle ? `@${c.claimer_handle}` : shortAddress(c.claimer_wallet)}
                  </div>
