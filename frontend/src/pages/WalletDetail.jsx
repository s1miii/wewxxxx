...
import { api, formatEth, formatUsd, shortAddress, timeAgo } from "@/lib/api";
...
export default function WalletDetail() {
  ...
  if (loading) return <div className="text-[#52525B] blink text-xs">Loading wallet</div>;
  if (err) return <div className="text-[#FF007A] text-sm">{err}</div>;
  if (!data) return null;

  return (
    <div className="space-y-6" data-testid="wallet-detail-page">
      ...
      <div className="panel corners p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <img src={data.avatar} alt={data.wallet} className="w-16 h-16 border border-[#00FF66] glow-green" />
            <div>
              <h1 className="text-head text-2xl font-bold text-white tracking-tighter">
                {data.handle ? `@${data.handle}` : "Anonymous Wallet"}
              </h1>
              <a
                href={`https://basescan.org/address/${data.wallet}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 mt-1 text-xs text-[#00F0FF] hover:underline font-mono"
              >
                {data.wallet} <ExternalLink size={10} />
              </a>
              {!data.handle && (
                <div className="text-[10px] text-[#52525B] italic mt-1">X handle not in Bankr registry</div>
              )}
            </div>
          </div>
          ...
          {data.claims.map((c) => (
            <div key={c.id} className="flex items-center gap-4 p-3 hover:bg-[#00FF66]/5">
              ...
              <div className="text-right">
                <div className="text-[#00FF66] font-semibold">{formatEth(c.amount)} {c.is_weth ? "Ξ" : ""}</div>
                <div className="text-[10px] text-[#52525B]">{c.is_weth ? formatUsd(c.amount_usd) : `$${c.token_symbol}`}</div>
              </div>
